...
export default function TextColor() {
  ...
  if (colorRef.current && alphaRef.current) {
    colorRef.current.value = rgbaColorToHex(textObject.color);
    alphaRef.current.value = String((textObject.color.a * 100).toFixed(2));
  }
  ...
  const handleChangeColor = () => {
    ...
    if (isColor(strColor)) {
      ...
      updateOverlayProperty<SceneText, "color">("color", newColor);
    }
  };
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="col-span-2 relative">
              <Input
                ref={colorRef}
                key={`${textObject.id}-text-color-rgb`}
                type="text"
                className="pl-9"
                defaultValue={rgbaColorToHex(textObject.color)}
            ...
            <Input
              ref={alphaRef}
              key={`${textObject.id}-text-color-alpha`}
              style={{ MozAppearance: "textfield" }} // should've work but nah
              type="number"
              min="10"
              max="100"
              defaultValue={String((textObject.color.a * 100).toFixed(2))}
              onBlur={handleChangeColor}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  handleChangeColor();
